...
class AlbumentationsCLAHE:
    def __init__(self, clahe_clip_limit=4.0, clahe_tile_grid_size=(8, 8)):
        self.transform = Compose([
            CLAHE(clip_limit=clahe_clip_limit, tile_grid_size=clahe_tile_grid_size)
        ])

    def __call__(self, img):
        img = np.array(img)  # 将 PIL 图像转换为 NumPy 数组
        img = self.transform(image=img)['image']
        return Image.fromarray(img)  # 将 NumPy 数组转换回 PIL 图像
...
def imshow(inp):
    """Imshow for Tensor without axes and title."""
...
def show_mixup_images(images, num_images=4):
    """在没有标题和坐标轴的情况下显示多张 mixup 图片"""
...
# 添加tensorboard
writer = SummaryWriter("./logs_train21")
...
for i in range(epoch):
    ...
    for data in train_dataloader:
        ...
        # 应用Mixup
        imgs, targets_a, targets_b, lam = mixup_data(imgs, targets, alpha=0.2, use_cuda=device == 'cuda')

        show_mixup_images(imgs)
        # 前向传播
        outputs = model(imgs)
    ...
    if auc > best_auc:
        best_auc = auc
        torch.save(model, "./model_pth/best_model21.pth")
        print("保存最佳模型")
