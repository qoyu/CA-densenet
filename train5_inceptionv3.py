...
num_classes = 2
resnet152 = torchvision.models.inception_v3(pretrained=True)  # 加载预训练模型
#resnet152.head = nn.Linear(resnet152.head.in_features, num_classes)  # 修改输出层
#print(resnet152)


resnet152.fc = nn.Linear(2048, num_classes)
#resnet152.classifier[6] = nn.Linear(4096, num_classes)
print(resnet152)
...
for i in range(epoch):
    ...
    for data in train_dataloader:
        ...
        #outputs = model(imgs)
        outputs, aux_outputs = model(imgs)  # 获取模型输出和辅助分类器的输出
    ...
    with torch.no_grad():  # with就是自带close()
        for data in val_dataloader:
            ...
            targets = targets.to(device)
            # outputs = model(imgs)
            outputs = model(imgs)  # 获取模型输出和辅助分类器的输出
            loss = loss_fn(outputs, targets)
