...
# 在数据加载器中使用 PuzzleMix
def puzzle_mix(batch_images, batch_labels):
    # 确保批量大小足够
    ...
    for i in range(len(batch_images)):
        # 选择另一个图像进行混合
        j = np.random.randint(len(batch_images))
        img1, img2 = batch_images[i], batch_images[j]
        label1, label2 = batch_labels[i], batch_labels[j]

        # 创建mask并进行混合
        mask = torch.rand(img1.size(1), img1.size(2), device=img1.device) > 0.5
        mask = mask.float().unsqueeze(0).repeat(3, 1, 1)
        mixed_image = img1 * mask + img2 * (1 - mask)

        # 选择权重更大的标签作为mixed_label
        mixed_label = label1 if mask.mean() > 0.5 else label2

        mixed_images.append(mixed_image)
        mixed_labels.append(mixed_label)

    return torch.stack(mixed_images), torch.tensor(mixed_labels, dtype=torch.long, device=batch_images.device)
...
#model = SE_DenseNet()
print(model)
model = ModifiedDenseNet().to(device) #传到GPU中去计算
...
# 添加tensorboard
writer = SummaryWriter("./logs_train19")
...
for i in range(epoch):
    ...
    for data in train_dataloader:
        ...
        # 应用 PuzzleMix
        mixed_imgs, mixed_targets = puzzle_mix(imgs, targets)
    ...
    if auc > best_auc:
        best_auc = auc
        torch.save(model, "./model_pth/best_model19.pth")
        print("保存最佳模型")
